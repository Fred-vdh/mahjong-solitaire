...
class MahjongGame:
    ...
    def format_time(self, ms):
        if ms is None or ms == 0 or ms > 99999999: return "--:--"
        sec = ms // 1000
    ...
    def recompute_scaling(self):
        ...
        for i, img_hd in enumerate(self.tile_images_hd):
            ...
            surf_3d = pygame.Surface((tw3d, th3d), pygame.SRCALPHA); rad_3d = max(3, int(self.tw / 12))
    ...
    def check_loss_condition(self):
        ...
        free_count = sum(1 for t in self.layout if t.get('free', self.is_free(t)))
        return free_count < 2
    ...
    def finalize_victory(self):
        ...
        lname = self.current_layout_name; stats = self.level_stats.get(lname, {})
    ...
    def update_sorted_layout(self):
        for t in self.layout:
            ...
            t['target_gray_alpha'] = 120.0 if (not t['free'] and getattr(self, 'show_gray_tiles', True)) else 0.0
    ...
    def make_solvable(self, types_pool=None):
        ...
        for _ in range(200):
            if self.construct_solvable_board(list(types_pool)): return
    ...
    def construct_solvable_board(self, full_tp):
        nt = len(self.tile_variants); seasons = [t for t in full_tp if nt - 8 <= t < nt - 4]; flowers = [t for t in full_tp if t >= nt - 4]; others = [t for t in full_tp if t < nt - 8]
    ...
    def get_score_target_pos(self):
        f_val = pygame.font.SysFont("Arial", 22, bold=True); score_str = str(self.displayed_score)
        val_w, val_h = f_val.size(score_str); return (110 + val_w // 2, 51 + val_h // 2)
    ...
    def start_shuffle_animation(self, start_pos=None):
        ...
        penalty = 5000; target_x, target_y = self.get_score_target_pos()
    ...
    def _execute_shuffle_logic(self):
        ...
        for t in self.layout:
            ...
            if found_idx != -1:
                ...
                self.shuffle_tiles_data.append({'type': target_type, 'start_pos': (sx, sy), 'current_pos': [sx, sy], 'end_pos': (ex, ey), 'tile': t, 'start_z': sz, 'end_z': ez, 'delay': random.uniform(0, 0.5), 'speed_mod': random.uniform(0.6, 1.4), 'arc_strength': random.uniform(50, 200), 'arc_dir': random.uniform(-1.0, 1.0), 'max_tilt': random.uniform(-45, 45), 'scale_mod': random.uniform(0.2, 0.5), 'rot': 0.0, 'scale': 1.0, 'arc_sin': 0.0})
    ...
    def update_shuffle_animation(self):
        if self.shuffle_anim_state == 'fading_out':
            ...
            total_prog = (pygame.time.get_ticks() - self.shuffle_start_time) / self.shuffle_duration; all_finished = True
            for d in self.shuffle_tiles_data:
                ...
                ease = 4 * tile_prog**3 if tile_prog < 0.5 else 1 - pow(-2 * tile_prog + 2, 3) / 2
                base_x = d['start_pos'][0] + (d['end_pos'][0] - d['start_pos'][0]) * ease; base_y = d['start_pos'][1] + (d['end_pos'][1] - d['start_pos'][1]) * ease
                arc_sin = np.sin(np.pi * tile_prog); d['arc_sin'] = arc_sin
                ...
                d['current_z'] = d['start_z'] + (d['end_z'] - d['start_z']) * tile_prog; d['rot'] = arc_sin * d['max_tilt']; d['scale'] = 1.0 + arc_sin * d.get('scale_mod', 0.4)
    ...
    def update_level_animations(self):
        if self.level_anim_state == 'in':
            ...
            if self.level_anim_progress == 1.0: self.level_anim_state = 'idle'; now = self.pause_start_ticks if self.is_paused else pygame.time.get_ticks(); self.start_ticks, self.last_match_time, self.last_move_time = now, now, now
    ...
    def update_ui_animations(self):
        if (self.won or self.lost) and self.win_ui_state == 'closed':
            if self.victory_anim_state == 'idle':
                if self.won: self.victory_anim_state = 'active'; self.victory_anim_start_time = pygame.time.get_ticks(); self.init_victory_animation()
                else: self.finalize_victory(); self.win_ui_state, self.win_ui_progress = 'opening', 0.0
            elif self.victory_anim_state == 'active':
                if self.win_ui_state == 'closed' and self.stats_ui_state == 'closed' and self.options_ui_state == 'closed' and pygame.time.get_ticks() - self.victory_anim_start_time > 3000: self.finalize_victory(); self.win_ui_state, self.win_ui_progress = 'opening', 0.0
    ...
    def update_animations(self):
        ...
        for s in self.flying_scores:
            s['progress'] += 0.01
            if s['progress'] < 1.0: rem_s.append(s)
    ...
    def update_history_animations(self):
        ...
        if self.undo_animating_tiles and all(a.get('finished') for a in self.undo_animating_tiles):
            if not hasattr(self, 'undo_settling_start') or self.undo_settling_start == 0: self.undo_settling_start = pygame.time.get_ticks()
            elif pygame.time.get_ticks() - self.undo_settling_start > 500:
                for a in self.undo_animating_tiles: self.layout.append(a['original_tile'])
                self.undo_animating_tiles = []; self.update_sorted_layout(); self.undo_settling_start = 0
        else: self.undo_settling_start = 0

    def init_victory_animation(self):
        self.victory_tiles = []; nv = len(self.std_tile_variants); cx, cy = self.width // 2, self.height // 2; anim_type = self.victory_anim_idx % 15
        for i in range(160):
    ...
    def update_victory_animation(self):
        ...
        is_draining = (self.victory_anim_state == 'draining'); cx, cy, nt = self.width // 2, self.height // 2, []
        for t in self.victory_tiles:
            t['rot'] += t['rot_speed']; v_anim = t['anim_type']
            if is_draining:
                if v_anim in (1, 3, 6, 10): t['dist'] += 12
                elif v_anim == 9: dx, dy = t['pos'][0] - cx, t['pos'][1] - cy; d = (dx**2 + dy**2)**0.5 or 1; t['pos'][0] += (dx/d) * 15; t['pos'][1] += (dy/d) * 15
                elif v_anim == 7: t['pos'][1] += 15; t['pos'][0] += t['vel'][0]
                elif v_anim == 11: t['pos'][0] += t['vel'][0]; t['vel'][1] += 0.4; t['pos'][1] += t['vel'][1]
                ...
                if v_anim == 0:
                    ...
                    wave = np.sin(t['angle'] * 2 + elapsed * 0.002) * 120; spread = (t['image_index'] % 8) * 30; target_dist = 250 + spread + wave; t['dist'] += (target_dist - t['dist']) * 0.05
                elif v_anim == 11:
            ...
            if not (is_draining and (t['pos'][1] > self.height + 200 or t['pos'][1] < -200 or t['pos'][0] > self.width + 200 or t['pos'][0] < -200)): nt.append(t)
        self.victory_tiles = nt
        if is_draining and len(self.victory_tiles) < 3:
            self.victory_tiles, self.victory_anim_state = [], 'completed'
            if self.queued_debug_anim is not None: self.victory_anim_idx = self.queued_debug_anim; self.queued_debug_anim = None; self.victory_anim_state, self.victory_anim_start_time = 'active', pygame.time.get_ticks(); self.init_victory_animation()
    ...
    def draw(self):
        ...
        if self.shuffle_anim_state == 'moving':
            for d in self.shuffle_tiles_data:
                tile_type, x, y = d['type'], d['current_pos'][0], d['current_pos'][1]; img = self.tile_variants[tile_type].get('3d', self.tile_variants[tile_type]['filtered']); h = d.get('arc_sin', 0.0); sh_off = 2 + int(h * 15); scale = d.get('scale', 1.0)
                if scale != 1.0:
                    sw, th = int(img.get_width() * scale), int(img.get_height() * scale)
        ...
        self.draw_cartridge(self.screen, pygame.Rect(self.width - tr_w - 20, 20, tr_w, tr_h)); self.screen.blit(f_sub.render("TUILES RESTANTES", True, (180, 180, 180)), (self.width - tr_w - 5, 30)); self.screen.blit(f_val.render(str(rem), True, (255, 255, 255)), (self.width - 78, 26)); poss_color = (150, 255, 150) if (poss > 0 or not self.layout) else (255, 100, 100); self.screen.blit(f_sub.render("COMBINAISONS", True, (180, 180, 180)), (self.width - tr_w - 5, 55)); self.screen.blit(f_val.render(str(poss), True, poss_color), (self.width - 78, 51))
        if self.shuffle_count > 0: self.screen.blit(f_sub.render("MÉLANGES", True, (180, 180, 180)), (self.width - tr_w - 5, 80)); self.screen.blit(f_val.render(str(self.shuffle_count), True, (255, 140, 0)), (self.width - 78, 76))
        ...
        if self.victory_tiles:
            for t in self.victory_tiles:
                img = self.tile_variants[t['image_index']].get('3d', self.tile_variants[t['image_index']]['filtered']); scale = t.get('scale', 1.0)
                if scale != 1.0:
        ...
        if self.win_ui_progress > 0:
            ...
            score_block_color = (60, 40, 40) if self.lost else (45, 45, 40); score_border_color = (150, 50, 50) if self.lost else (218, 165, 32); pygame.draw.rect(win_surf, score_block_color, (pw-310, 185, 230, 110), border_radius=15); pygame.draw.rect(win_surf, score_border_color, (pw-310, 185, 230, 110), 2, border_radius=15); win_surf.blit(f_label.render("SCORE", True, score_border_color), (pw-295, 195)); s_val = f_highlight.render(str(self.current_score), True, (255, 255, 255)); win_surf.blit(s_val, s_val.get_rect(center=(pw-195, 235))); best_score = stats.get("prev_best_score")
            if not self.lost:
                if stats.get("is_new_score_record"): win_surf.blit(f_record.render("NOUVEAU RECORD !", True, (255, 215, 0)), (pw-295, 265))
            ...
            for i, (label, b_id) in enumerate(btn_configs):
                bx = start_bx + i * (bw + gap); rect = pygame.Rect(bx, 450, bw, bh); is_h = rect.collidepoint(adj_m_p) if self.win_ui_state == 'open' else False; is_p = getattr(self, 'win_pressed_btn', None) == b_id and is_h; b_color = (60, 179, 113) if b_id=="random" else (70, 130, 180); self.draw_button(win_surf, rect, label, b_color, f_btn, is_h, is_p, border_radius=15); self.win_btn_rects.append({'rect': pygame.Rect(sx + rect.x * scale, sy + rect.y * scale, rect.w * scale, rect.h * scale), 'id': b_id})
    ...
    def sort_stats(self, column):
        ...
        def sort_key(idx):
            name, s = self.layout_names[idx], self.level_stats.get(self.layout_names[idx], {})
            if column == "Niveau": return self.normalize_string(name)
            elif column == "T. Min": val = s.get("best_time", 999999999); return val if val is not None else 999999999
            elif column == "Mél. Min": val = s.get("best_shuffles", 999); return val if val is not None else 999
            elif column == "Ind. Min": val = s.get("best_hints", 999); return val if val is not None else 999
            elif column == "Victoires": return s.get("times_completed", 0)
            elif column == "Score": return s.get("best_score", 0)
            return 0
    ...
    def draw_button(self, surface, rect, text, base_color, font, is_hover, is_pressed, border_radius=10):
        ...
        draw_color = [min(255, c + 30) for c in base_color] if is_hover else base_color; pygame.draw.rect(surface, draw_color, btn_rect, border_radius=border_radius)
    ...
    def draw_stats_ui(self):
        ov = pygame.Surface((self.width, self.height), pygame.SRCALPHA); ov.fill((0, 0, 0, int(200 * self.stats_ui_progress))); self.screen.blit(ov, (0, 0)); sw, sh = 800, 600; scale = 0.8 + 0.2 * self.stats_ui_progress; dsw, dsh = int(sw * scale), int(sh * scale); sx, sy = (self.width - dsw) // 2, (self.height - dsh) // 2; m_pos = pygame.mouse.get_pos(); adj_m_pos = ((m_pos[0] - sx) / scale, (m_pos[1] - sy) / scale); stats_surf = pygame.Surface((sw, sh), pygame.SRCALPHA); pygame.draw.rect(stats_surf, (30, 30, 35, 255), (0, 0, sw, sh), border_radius=20); pygame.draw.rect(stats_surf, (218, 165, 32, 255), (0, 0, sw, sh), 4, border_radius=20); f_title, f_header, f_row, f_arrow_icon = pygame.font.SysFont("Arial", 32, True), pygame.font.SysFont("Arial", 18, True), pygame.font.SysFont("Arial", 15), pygame.font.SysFont("Arial", 12, True); t_title = f_title.render("Statistiques par Niveau", True, (255, 255, 255)); stats_surf.blit(t_title, t_title.get_rect(centerx=sw//2, y=30)); hy = 90; header_configs = [("Aperçu", 40, 120, None), ("Niveau", 170, 180, "Niveau"), ("T. Min", 360, 70, "T. Min"), ("Mél.", 440, 50, "Mél. Min"), ("Ind. Min", 500, 70, "Ind. Min"), ("Victoires", 580, 80, "Victoires"), ("Score", 670, 80, "Score")]; self.stats_header_rects = []
        ...
        for display_idx in range(start_idx, end_idx):
            i = self.stats_display_indices[display_idx]; name, ry = self.layout_names[i], display_idx * row_h - self.stats_scroll_y; preview = self.level_previews.get(name); ix, iy, iw, ih = sx + clip_rect.x * scale, sy + (clip_rect.y + ry) * scale, clip_rect.width * scale, row_h * scale; g_clip_top, g_clip_bottom = sy + clip_rect.y * scale, sy + (clip_rect.y + clip_rect.height) * scale; final_y = max(iy, g_clip_top); final_h = min(iy + ih, g_clip_bottom) - final_y
            if final_h > 0: self.stats_rows_rects.append({'rect': pygame.Rect(ix, final_y, iw, final_h), 'level_index': i})
            is_pressed = (self.pressed_stat_row == i); row_content_off = 3 if is_pressed else 0
            if is_pressed: pygame.draw.rect(rows_view, (20, 20, 25, 255), (0, ry, clip_rect.width, row_h))
            pygame.draw.line(rows_view, (60, 60, 70), (0, ry + row_h), (clip_rect.width, ry + row_h), 1)
            if preview: rows_view.blit(preview, (0, ry + row_content_off))
            rows_view.blit(f_row.render(name, True, (220, 220, 220)), (130, ry + 30 + row_content_off)); s = self.level_stats.get(name)
            if s:
                bt = s.get("best_time"); time_str = f"{(bt//1000)//60:02}:{(bt//1000)%60:02}" if bt and 0 < bt < 999999999 else "--:--"
                t_surf = f_row.render(time_str, True, (0, 255, 127)); rows_view.blit(t_surf, (320 + (70 - t_surf.get_width()) // 2, ry + 30 + row_content_off))
                bs_surf = f_row.render(str(s.get("best_shuffles", 0)), True, (255, 165, 0)); rows_view.blit(bs_surf, (400 + (50 - bs_surf.get_width()) // 2, ry + 30 + row_content_off))
                bh_val = s.get("best_hints", 999); bh_str = str(bh_val) if bh_val != 999 else "--"
                bh_surf = f_row.render(bh_str, True, (180, 120, 220)); rows_view.blit(bh_surf, (460 + (70 - bh_surf.get_width()) // 2, ry + 30 + row_content_off))
                tc_surf = f_row.render(str(s.get("times_completed", 0)), True, (100, 200, 255)); rows_view.blit(tc_surf, (540 + (80 - tc_surf.get_width()) // 2, ry + 30 + row_content_off))
                sc_surf = f_row.render(str(s.get("best_score") or 0), True, (255, 215, 0)); rows_view.blit(sc_surf, (630 + (80 - sc_surf.get_width()) // 2, ry + 30 + row_content_off))
            else:
                t_surf = f_row.render("--:--", True, (100, 100, 100)); rows_view.blit(t_surf, (320 + (70 - t_surf.get_width()) // 2, ry + 30 + row_content_off))
                bs_surf = f_row.render("-", True, (100, 100, 100)); rows_view.blit(bs_surf, (400 + (50 - bs_surf.get_width()) // 2, ry + 30 + row_content_off))
                bh_surf = f_row.render("-", True, (100, 100, 100)); rows_view.blit(bh_surf, (460 + (70 - bh_surf.get_width()) // 2, ry + 30 + row_content_off))
                tc_surf = f_row.render("0", True, (100, 100, 100)); rows_view.blit(tc_surf, (540 + (80 - tc_surf.get_width()) // 2, ry + 30 + row_content_off))
                sc_surf = f_row.render("0", True, (100, 100, 100)); rows_view.blit(sc_surf, (630 + (80 - sc_surf.get_width()) // 2, ry + 30 + row_content_off))
    ...
    def draw_options_ui(self):
        ...
        opt_y = start_y + len(opt_configs) * spacing; slider_w, slider_h = 250, 10; vol_configs = [("Volume de la musique :", 'music_volume', 'music_slider_rect', 'music_knob_rect'), ("Volume des effets :", 'sfx_volume', 'sfx_slider_rect', 'sfx_knob_rect')]
        ...
        if self.stats_ui_state == 'open':
            ...
            if hasattr(self, 'stats_rows_rects'):
                for row in self.stats_rows_rects:
                    if row['rect'].collidepoint(pos): self.play_click_sfx(); self.pressed_stat_row = row['level_index']; return
    ...
    def handle_release(self, pos):
        ...
        if self.pressed_stat_row is not None:
            row_idx = self.pressed_stat_row; released_on_row = False
            if hasattr(self, 'stats_rows_rects'):
                for row in self.stats_rows_rects:
                    if row['level_index'] == row_idx and row['rect'].collidepoint(pos): released_on_row = True; break
            if released_on_row: self.pending_level_idx = row_idx; self.row_click_time = pygame.time.get_ticks(); return
            else: self.pressed_stat_row = None; return
                ...
                if hasattr(self, 'win_btn_rects'):
                    for b in self.win_btn_rects:
                        if b['id'] == btn_id and b['rect'].collidepoint(pos):
                            if btn_id == 'random': self.next_level_to_load = None; self.victory_anim_state = 'draining'
        ...
        if pb == "options_close" and hasattr(self, 'options_close_btn_global') and self.options_close_btn_global.collidepoint(pos): self.options_ui_state = 'closing'
        ...
        elif pb == "manual_shuffle" and hasattr(self, 'manual_shuffle_btn_rect') and self.manual_shuffle_btn_rect.collidepoint(pos): self.start_shuffle_animation(self.manual_shuffle_btn_rect.center)
            ...
            if h:
                p = 500; tx, ty = self.get_score_target_pos(); sx, sy = self.hint_btn_rect.center; self.flying_scores.append({'pos': [sx, sy], 'target': (tx, ty), 'text': f"-{p}", 'score_value': -p, 'progress': 0.0})
                self.hint_pair, self.last_move_time = h, pygame.time.get_ticks(); self.hint_count += 1
